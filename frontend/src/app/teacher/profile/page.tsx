...
export default function TeacherProfilePage() {
  ...
  return (
    <AppShell
      role="teacher"
      title="Profile"
      subtitle="Manage your faculty information and notification settings."
    >
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <Panel>
          <PanelHeader
            title="Faculty Details"
            subtitle="Update the information students see across the portal."
          />
          <div className="grid gap-4 p-6 md:grid-cols-2">
            ...
            <Input label="Designation" value={data?.profile.designation ?? ''} onChange={(value) => setData((prev) => (prev ? { ...prev, profile: { ...prev.profile, designation: value } } : prev))} />
            <Input label="Location" value={data?.profile.location ?? ''} onChange={(value) => setData((prev) => (prev ? { ...prev, profile: { ...prev.profile, location: value } } : prev))} />
            ...
            <div className="md:col-span-2">
              <div className="flex gap-3">
                <button
                  ...
                  onClick={async () => {
                    if (!data) return;
                    await teacherApi.requestProfileUpdate(data);
                    setMessage('Profile change request sent to admin for review.');
                    setInitialData(data);
                  }}
        ...
        <Panel>
          <PanelHeader
            title="Settings"
            subtitle="Choose how SmartClass 360 keeps you updated."
          />
